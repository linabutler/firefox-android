...
class FxSuggestSuggestionProviderTest {
    ...
    fun inputEmpty() = runTest {
        whenever(suggestStore.query(any())).thenReturn(listOf(
        ...
        verify(suggestStore, never()).query(any())
    ...
    fun inputNotEmpty()  = runTest {
        ...
        verify(suggestStore).query(eq(SuggestionQuery(
            keyword = "la",
    ...
    fun inputCancelled() = runTest {
        doNothing().`when`(suggestStore).interrupt()
